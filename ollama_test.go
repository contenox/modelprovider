...
func TestSystem_Ollama(t *testing.T) {
	...
	ctx := t.Context()
	uri, _, cleanup, err := modelprovider.SetupOllamaLocalInstance(ctx, "latest")
	...
	t.Run("Smoke", func(t *testing.T) {
		...
		caps := modelprovider.CapabilityConfig{
			ContextLength: 2048,
			CanChat:       true,
			CanEmbed:      true,
			CanStream:     true,
		...
		provider := modelprovider.NewOllamaModelProvider(chatModel, []string{uri}, http.DefaultClient, caps)
	...
	t.Run("WithOptions", func(t *testing.T) {
		...
		resp, err := chatClient.Chat(ctx, messages,
			...
			modelprovider.WithTemperature(0.1),
			modelprovider.WithMaxTokens(10))
		require.NoError(t, err)
	...
	t.Run("BasicEmbedding", func(t *testing.T) {
		...
		provider := modelprovider.NewOllamaModelProvider(embedModel, []string{uri}, http.DefaultClient, caps)
